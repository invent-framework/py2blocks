...
if __name__ == "__main__":
    ...
    py2blocks_observer = Observer()
    test_observer = Observer()
    py2blocks_observer.schedule(event_handler, "src", recursive=True)
    test_observer.schedule(event_handler, "tests", recursive=True)
    py2blocks_observer.start()
    test_observer.start()
    try:
        while True:
            time.sleep(1)
        ...
        py2blocks_observer.stop()
        test_observer.stop()
    py2blocks_observer.join()
    test_observer.join()
